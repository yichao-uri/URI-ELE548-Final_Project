...
//	vector_out_length = 128;
	...
	sigmoid_switch = 1;
//	sigmoid_switch = 0;
	stream_sigmoid_switch.write(sigmoid_switch);
...
//			stream_weight_in.write(data_w_t(1.0/vector_in_length*float(0.1*i2)));
			stream_weight_in.write(data_w_t(0));
//			stream_weight_in.write(data_w_t(i1*0.01));
		}
	}
	for(i1 = 0; i1 < 100000000; i1++){
		RBM(stream_control_in, stream_sigmoid_switch, vector_in_len, vector_out_len, stream_vector_in, stream_weight_in, stream_bias_in, stream_vector_out);
...
//		stream_bias_in.write(data_b_t(0));
		stream_bias_in.write(data_b_t(-65535));
	}
	for(i1 = 0; i1 < 100000000; i1++){
		RBM(stream_control_in, stream_sigmoid_switch, vector_in_len, vector_out_len, stream_vector_in, stream_weight_in, stream_bias_in, stream_vector_out);
	}
	...
	for(i1 = 0; i1 < (N); i1++){
		stream_data_v.data = -1000;
//		stream_data_v.data = i1%vector_in_length;
